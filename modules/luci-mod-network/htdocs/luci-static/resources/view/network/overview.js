...
return view.extend({
	load: function() {
		return Promise.all([
			L.require('view.status.include.30_network'),
			L.require('view.status.include.40_dhcp'),
			L.require('view.status.include.50_dsl'),
			L.require('view.status.include.60_wifi')
		]);
	},
	...
	render: function(includes) {
		...
		console.log("YYYYYY INCLUDES", includes);
